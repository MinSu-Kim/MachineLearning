...
if __name__ == "__main__":
    ...
    print("# 전체 데이터 중 80%는 학습용 데이터, 20%는 테스트용 데이터 추출")
    print(X_train.shape, X_test.shape, y_train.shape, y_test.shape, end='\n\n')

    # 위에서 만든 X_train, y_train을 다시 쪼개서 90%는 학습과 10%는 검증용 데이터로 분리
    X_tr, X_val, y_tr, y_val = train_test_split(X_train, y_train, test_size=0.1, random_state=156)
    print("# 위에서 만든 X_train, y_train을 다시 쪼개서 90%는 학습과 10%는 검증용 데이터로 분리")
    ...
    print('predict( ) 수행 결과값을 10개만 표시, 예측 확률 값으로 표시됨', end='\n\n')
    ...
    # %matplotlib inline -> plt.show() pycharm실행시 변경
